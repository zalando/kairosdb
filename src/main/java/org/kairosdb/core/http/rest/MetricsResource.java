...
public class MetricsResource implements KairosMetricReporter {
    ...
    private final String baseCacheDir = System.getProperty("java.io.tmpdir") + "/kairos_cache/";
    ...
    public Response getMeta(String json) {
        ...
        try {
            File respFile = File.createTempFile("kairos", ".json", new File(baseCacheDir));
    ...
    public Response get(String json) throws Exception {
        ...
        try {
            ...
            for (QueryMetric query : queries) {
                QueryCallback callback = new DumbQueryCallback(query.getName());
            ...
            logger.error("Failed to open temp folder " + baseCacheDir, e);
            return setHeaders(Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new ErrorResponse(e.getMessage()))).build();
        } catch (Exception e) {
            logger.error("Query failed.", e);
            return setHeaders(Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new ErrorResponse(e.getMessage()))).build();

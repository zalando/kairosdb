...
public class HealthCheckResource {
    ...
    private HealthCheckMetricProvider healthCheckMetricProvider;
    ...
    public HealthCheckResource(HealthCheckService healthCheckService,
                               HealthCheckMetricProvider healthCheckMetricProvider) {
        m_healthCheckService = checkNotNull(healthCheckService);
        this.healthCheckMetricProvider = healthCheckMetricProvider;
    }
    ...
    public Response check() {
        for (HealthStatus healthCheck : m_healthCheckService.getChecks()) {
            ...
            if (!result.isHealthy()) {
                switch (healthCheck.getName()) {
                    case "Datastore-Query":
						healthCheckMetricProvider.measureCassandraUnhealthyCheck();
                        break;
                    case "JVM-Thread-Deadlock":
						healthCheckMetricProvider.measureJVMDeadlockThread();
                        break;
                }
